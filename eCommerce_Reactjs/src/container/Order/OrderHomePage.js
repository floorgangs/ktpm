...
function OrderHomePage(props) {
  ...
  const handleGHNAddressChange = useCallback(
    async (addressData) => {
      ...
      if (
        ...
        const currentAddress = dataAddressUser.find(
          (addr) => addr.id === addressUserId
        );
  ...
  const handleSelectVoucher = (voucher) => {
    const currentPrice = dataCart.reduce(
      (sum, item) => sum + item.quantity * item.productDetail.discountPrice,
      0
    );

    if (voucher.typeVoucherData.minValue > currentPrice) {
      toast.error(
        `Đơn hàng tối thiểu ${CommonUtils.formatter.format(
          voucher.typeVoucherData.minValue
  ...
  const getApplicableVouchers = useCallback(() => {
    ...
    const now = new Date();
    const productIds = dataCart
      .map((item) => item.productDetail?.productId)
      .filter(Boolean);

    return voucherWallet
      .map((item) => {
        const voucher = item.voucherData;
        if (!voucher || !voucher.typeVoucherData)
          return { ...voucher, isApplicable: false };
        ...
        if (voucher.applicableProducts) {
          try {
            const applicableProducts =
              typeof voucher.applicableProducts === "string"
            ...
            if (
              ...
              isProductApplicable = productIds.some((pid) =>
  ...
  let handleSaveOrder = async () => {
    ...
    if (!currentAddress?.districtName || !currentAddress?.wardName) {
      toast.error(
        "Địa chỉ chưa có thông tin Tỉnh/Huyện/Xã. Vui lòng cập nhật địa chỉ."
      );
      return;
    }
    ...
    if (activeTypePayment === 0) {
      let res = await createNewOrderService({
        ...
        note: note,
        // userId: userId,  // ← Xoá dòng này, backend sẽ lấy từ token
                ...
                <>
                  {dataAddressUser && dataAddressUser.length > 0 ? (
                    <>
                      <div className="content-left">
                        <span>
                          {dataAddressUser[stt].name} (
                          {dataAddressUser[stt].phonenumber})
                        </span>
                      </div>
                      <div className="content-center">
                        <span>{dataAddressUser[stt].address}</span>
                      </div>
                    </>
                  ) : (
                    <div className="content-left">
            ...
            <div className="container">
              ...
              <div className="box-shipping">
                <h6 style={{ marginBottom: "15px" }}>
                  <i
                    ...
                    style={{ marginRight: "10px", color: "#ee4d2d" }}
                  ></i>
                ...
                {!ghnInitialAddress &&
                  ...
                  dataAddressUser.length > 0 && (
                    ...
                    >
                      ...
                      ></i>
                      Địa chỉ "<b>{dataAddressUser[stt]?.name}</b>" chưa có
                ...
                <div
                  className={`shipping-option ${
                    ghnInitialAddress ? "active" : "disabled"
                  }`}
                  style={{
                    border: ghnInitialAddress
                      ? "2px solid #ee4d2d"
                      : "1px solid #d9d9d9",
                    borderRadius: "8px",
                    padding: "15px",
                  ...
                  {ghnInitialAddress &&
                    ...
                    dataAddressUser.length > 0 && (
                      ...
                      >
                        <i className="fas fa-map-marker-alt"></i>
                        <span>
                          Giao đến: {dataAddressUser[stt]?.wardName},{" "}
                          {dataAddressUser[stt]?.districtName},{" "}
                          {dataAddressUser[stt]?.provinceName}
                        </span>
                      </div>
                    )}
                </div>
              ...
              <div className="box-voucher">
                ...
                <div className="voucher-content">
                    ...
                    <div className="selected-voucher">
                      <div className="voucher-info">
                        <span className="voucher-code">
                          {selectedVoucher.codeVoucher}
                        </span>
                        <span className="voucher-discount">
                          {selectedVoucher.typeVoucherData?.typeVoucher ===
                          "percent"
                            ? `Giảm ${selectedVoucher.typeVoucherData.value}%`
                            : `Giảm ${CommonUtils.formatter.format(
                                selectedVoucher.typeVoucherData?.value
                              )}`}
                        </span>
                      </div>
                      <button
                  ...
                  <button
                    ...
                    onClick={() => setIsShowVoucherModal(true)}
                  >
              ...
              {isShowVoucherModal && (
                <div
                  ...
                  onClick={() => setIsShowVoucherModal(false)}
                >
                  <div
                    ...
                    <div className="voucher-modal-body">
                      {getApplicableVouchers().length > 0 ? (
                          ...
                          >
                            <div className="voucher-item-left">
                              <div className="voucher-badge">
                                {voucher.typeVoucherData?.typeVoucher ===
                                "percent"
                            ...
                            <div className="voucher-item-right">
                              ...
                              <div className="voucher-item-desc">
                                ...
                                "percent"
                                  ? `Giảm ${
                                      voucher.typeVoucherData.value
                                    }% - Tối đa ${CommonUtils.formatter.format(
                                      voucher.typeVoucherData.maxValue
                                    )}`
                                  : `Giảm ${CommonUtils.formatter.format(
                                      voucher.typeVoucherData?.value
                                    )}`}
                              ...
                              <div className="voucher-item-expire">
                                HSD:{" "}
                                {new Date(voucher.toDate).toLocaleDateString(
                                  "vi-VN"
                                )}
                              </div>
                              {!voucher.isApplicable && (
                        ...
                        <div className="no-voucher-available">
                          ...
                          <NavLink to={`/user/voucher/${userId}`}>
                            Lấy voucher ngay
                          </NavLink>
                        </div>
                      )}
                ...
                <div className="content-right">
                  <div className="wrap-price">
                    ...
                    <span className="text-price">
                      {CommonUtils.formatter.format(
                        price + +priceShip - discountAmount
                      )}
                    </span>
                  </div>
                </div>
              ...
              {discountAmount > 0 && (
                <div className="box-flex voucher-discount">
                  ...
                  <div className="discount-value">
                    -{CommonUtils.formatter.format(discountAmount)}
                  </div>
                </div>
              )}
              <div className="box-flex">
